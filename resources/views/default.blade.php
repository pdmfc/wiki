...
<html>
    <head>
        ...
        @foreach(\Pdmfc\Wiki\Wiki::allStyles() as $name => $path)
            @if (preg_match('/^https?:\/\//', $path))
                <link rel="stylesheet" href="{{ $path }}">
            @else
                <link rel="stylesheet" href="{{ route('wiki.styles', $name) }}">
            @endif
        @endforeach
    ...
    <body>
        ...
        @foreach (\Pdmfc\Wiki\Wiki::allScripts() as $name => $path)
            @if (preg_match('/^https?:\/\//', $path))
                <script src="{{ $path }}"></script>
            @else
                <script src="{{ route('wiki.scripts', $name) }}"></script>
            @endif
        @endforeach
