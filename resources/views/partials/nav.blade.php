                    ...
                    <ul class="list-reset">
                        @foreach ($roles as $role)
                            <li class="py-2 hover:bg-grey-lightest">
                                <a class="px-6 text-grey-darkest whitespace-no-wrap" href="{{ route('wiki.show', ['version' => $currentVersion, 'role' => $role['slug'], 'page' => $currentSection]) }}">{{ $role['name'] }}</a>
                            </li>
                        @endforeach
                    </ul>
                </template>
