    ...
    <div class="internal-autocomplete-result">
      <ul v-if="filteredPages.length">
        ...
        >
          ...
          <p
            class="heading"
            v-for="(heading, idx) in page.headings"
            :key="idx"
            v-if="searchInHeading(heading.text)"
            @click="navigateToHeading(page, heading.link)"
          >{{ heading.text }}</p>
        </li>
      </ul>

      <div
        v-if="! filteredPages.length && isLoaded"
...
export default {
  ...
  methods: {
    ...
    navigateToHeading(page, link) {
      window.location = this.versionUrl + page.path + "#" + link;
    },
    searchInHeading(heading) {

      if(this.search.length < 1) {
        return true;
      }

      return heading.toLowerCase().includes(this.search.toLowerCase())
    }
  },
  computed: {
    filteredPages() {
      ...
      return this.pages.filter(page => {
        ...
        page.headings.forEach(heading => {
          if (heading.text.toLowerCase().includes(this.search)) {
            foundInHeading = true;
          }
        });
