...
class DocumentationController extends Controller
{
    ...
    public function show($version, $role, $page = null)
    {
        ...
        $permissions = Permission::where('is_document', 1)
            ->where('role_id', auth()->user()->role->id)
            ->where('view', 1)
            ->pluck('resource')
            ->toArray();

        $roles = Role::where('market_id', 2)->get();
        $rolePermissions = [];
        foreach ($roles as $row) {
            if( in_array(Str::ucfirst(Str::camel($row->name)), $permissions) ){
                $rolePermissions[] = [
                    'name' => $row->name,
        ...
        $currentRolePermissions = str_replace(' ', '', ucwords(str_replace('-', ' ', $role)));

        if(!in_array($currentRolePermissions, $permissions, true)) {
